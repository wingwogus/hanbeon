...
Wave 2의 활성 앱 조회는 macOS bundle ID, Windows 실행 파일 basename, Linux X11
desktop ID와 `WM_CLASS`를 원시 값으로 읽는다. 기존 macOS 내장 프리셋은 bundle ID를
계속 사용한다. Windows·Linux X11 식별자를 프리셋으로 인식·적용하거나 레지스트리를
다운로드·캐싱하는 일은 이 Wave에 포함하지 않는다. Linux Wayland에서 안전한
compositor/desktop 어댑터가 없으면 앱별 프리셋을 선택하지 않고 기본 4칸을 유지한다.
